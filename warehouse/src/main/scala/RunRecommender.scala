...
class RunRecommender(private val spark: SparkSession) {
  ...
  def model(
    ...
    artistByID.filter($"id" isin (existingArtistIDs:_*)).show(5)
    ...
    model.transform(toRecommend).show(6)

    //toRecommend.select("artist").as[Int].collect()
  ...
  def areaUnderCurve(
    ...
    // Take held-out data as the "positive".None
